...
int main(){
	...
	while(1){
		...
		switch(codigo_coordi){
		case 20:
			recibir(socket_Coordinador, &exito, sizeof(int), logger_esi);
			resultados_esi resultado;
			if(exito){
				resultado = EXITO;
			} else resultado = FALLO;
			enviar(socket_Planificador, &resultado, sizeof(resultados_esi), 41, logger_esi);
		}
	}
}
